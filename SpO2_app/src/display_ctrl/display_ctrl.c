...
/***	DisplayInitialize(DisplayCtrl *dispPtr, XAxiVdma *vdma, UINTPTR vtcBaseAddr, u32 dynClkAddr, u8 *framePtr[DISPLAY_NUM_FRAMES], u32 stride)
**
**	Parameters:
**		dispPtr - Pointer to the struct that will be initialized
**		vdma - Pointer to initialized VDMA struct
**		vtcBaseAddr - VTC base address from xparameters.h when using Vitis SDT flow
...
**
*/
int DisplayInitialize(DisplayCtrl *dispPtr, XAxiVdma *vdma, UINTPTR vtcBaseAddr, u32 dynClkAddr, u8 *framePtr[DISPLAY_NUM_FRAMES], u32 stride)
{
	int Status;
	int i;
...
int DisplayChangeFrame(DisplayCtrl *dispPtr, u32 frameIndex)
{
	...
	{
		xdbg_printf(XDBG_DEBUG_GENERAL, "Invalid frame index %lu\r\n", (unsigned long)frameIndex);
		return XST_FAILURE;
	}
